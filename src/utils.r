...
bash_script = function(
    ...
    roadmap_bed = TRUE, # Read roadmap_bed files
    debug = FALSE
) {
    # Preparing...
    ...
    if (roadmap_bed) {
      out_dir = paste0(out, '/roadmap_dist_25status/')
    } else {
      out_dir = paste0(out, '/roadmap_dist/')
    }
    out2  paste0(out_dir, tools::file_path_sans_ext(roadmap_f_name), '.tsv')
    bash2 = paste0(
